    ...
    def update_cmd_msg(self):
        ...
        if self.state.value[:-2] == "driving":
            if self.odometry.x != 0.0:  # 10.03 수정
                ...
                steer, self.target_idx = self.pp.pure_pursuit_control(
                    self.odometry, self.path.cx, self.path.cy
                )
                ...
                speed = self.pid.PIDControl(
                    self.odometry.v * 3.6, target_speed, min=5, max=15
                )  # speed 조정 (PI control)
            ...
            if self.odometry.x != 0.0:  # 10.03 수정
                ...
                target_speed = self.set_target_speed()
                # adapted_speed = self.ss.adaptSpeed(
                #     target_speed, hdr, ctr, min_value=6, max_value=10
                # )  # 에러(hdr, ctr) 기반 목표 속력 조정
                speed = self.pid.PIDControl(
