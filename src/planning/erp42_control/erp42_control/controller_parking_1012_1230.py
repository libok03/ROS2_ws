...
class Pakring():
    def __init__(self,node):
        ...
        self.search_path_db = DB("1009_2212_bunsudae_search_path.db") #dolge
        ...
        self.parking_path_db = DB("1009_2211_parking_path.db")
        ...
        self.min_x = self.standard_point.x +0.0
        self.max_x = self.standard_point.x + 28.0
        self.min_y = self.standard_point.y -3.7
    ...
    def adjust_parking_path(self):
        ...
        goal_pose = self.rotate_points(
            np.array(
                [self.low_y_cone[self.idx][0] + 1.9, self.low_y_cone[self.idx][1] - 1.5]
            ),
            -self.standard_point.yaw,
            np.array([self.standard_point.x, self.standard_point.y]),
        )
    ...
    def control_parking(self,State):
        ...
        if self.target_idx >= len(self.path_cx) - self.goal - 1:
            ...
            if self.parking_state == Parking_state.STOP:
                ...
                self.path_cx = self.search_path[:, 0]  # 첫 번째 열 (cx 값들)
                self.path_cy = self.search_path[:, 1]  # 두 번째 열 (cy 값들)
                self.path_cyaw = self.search_path[:, 2]  # 세 번째 열 (cyaw 값들)
                self.path_for_visu()
                msg = ControlMessage(mora=0, estop=1,gear=0,speed = 0*10, steer = 0,brake=200)
            ...
            # SEARCH, PARKING, RETURN
                ...
                try:
                    ...
                    msg = ControlMessage(mora=0, estop=0,gear=0,speed = speed*10, steer = int(m.degrees(-1* steer)),brake=0)
                except Exception as e:
                    print(f"Stanley:{e}\n",f"{State}")
                return msg, False
                ...
                try:
                    ...
                    target_speed = 8.0
                    adapted_speed = self.ss.adaptSpeed(target_speed, hdr, ctr, min_value=6, max_value=10
                    )
                    speed = self.pid.PIDControl(State.v * 3.6, adapted_speed)
                    msg = ControlMessage(mora=0, estop=0,gear=2,speed = speed*10, steer = int(m.degrees(-1* steer)),brake=0)
