...
class Obstacle:
    ...
    def timer_callback(self):
        ...
        if (
            ...
            if (self.odometry.x - 84.98333740234375) ** 2 + (
                self.odometry.y - 90.80550384521484
            ) ** 2 < (self.odometry.x - 124.6465275631675) ** 2 + (
                self.odometry.y - 239.69487719818875
            ) ** 2:
                # k-city2 @ 작은거
                self.publish_ref_path(
                    wx=[83.27738189697266,63.86278533935547],
                    wy=[90.05860900878906, 95.38835906982422],
                    num=1,
                )  # 1차선 center line

                self.publish_ref_path(
                    wx=[84.98333740234375, 58.269317626953125],
                    wy=[90.80550384521484, 101.64997100830078],
                    num=2,
                )  # 2차선 center line
                ...
                self.publish_ref_path(
                    wx=[118.86103820800781,  139.983,],
                    wy=[223.68418884277344,  266.446,],
                    num=2,
                )  # 2차선 center line
