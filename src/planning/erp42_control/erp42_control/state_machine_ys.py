        ...
        if self.state.value[:-2] == "driving":
            if self.odometry.x != 0.0:  # 10.03 수정
                ...
                speed = self.pid.PIDControl(
                    self.odometry.v * 3.6, adapted_speed, min=5, max=25
                )  # speed 조정 (PI control)
            ...
            if self.odometry.x != 0.0:  # 10.03 수정
                ...
                target_speed = self.set_target_speed()
                adapted_speed = self.ss.adaptSpeed(
                    target_speed, hdr, ctr, min_value=6, max_value=20
                )  # 에러(hdr, ctr) 기반 목표 속력 조정
                speed = self.pid.PIDControl(
                    self.odometry.v * 3.6, adapted_speed, min=6, max=20
                )  # speed 조정 (PI control)
                brake = self.cacluate_brake(adapted_speed)  # brake 조정
...
def main():
    ...
    node.declare_parameter("file_name", "k_city_ys_v1_new.db")
