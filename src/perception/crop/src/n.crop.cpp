...
public:
    VelodyneSubscriber() : Node("bs_cropbox_filter")
    {
        ...
        cropbox_publisher_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/new_cropbox_filtered", 10);
    }
...
private:
    void velodyneCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
        ...
        const float min_y = 0;   //-1.8
        const float max_y = 2;    //1.8
...
//             } else if (field.name == "y") {
//                 y_offset = field.offset;
//             } else if (field.name == "z") {  // Find z offset
...
//         const size_t float_size = 4;
//         const float min_x = 0.8;
//         const float max_x = 2.3;
//         const float min_y = -1.6;
//         const float max_y = 1.6;
//         const float min_z = -1.0;  // Define min and max z values
//         const float max_z = 3.0;
