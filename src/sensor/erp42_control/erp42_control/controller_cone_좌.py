...
class Drive():
    ...
    def publish_cmd(self):
        ...
        if self.first_lap: # first lap
            ...
            if self.decision_straight(target_idx):
                ...
                steer, hdr, ctr = self.st.stanley_control(self.state, self.path.cyaw, h_gain_straight, c_gain_straight, target_idx, error)
                adapted_speed = self.ss.adaptSpeed(target_speed, hdr, ctr, min_value=4, max_value=15, he_gain=40.0, ce_gain=20.0, he_thr=0.001, ce_thr=0.002)
                if self.state.v * 3.6 >= adapted_speed:
                    input_brake = (abs(self.state.v * 3.6 - adapted_speed) / 20.0) * 200
                else:
                    input_brake = 0
                ...
                steer, hdr, ctr = self.st.stanley_control(self.state, self.path.cyaw, h_gain_curve, c_gain_curve, target_idx, error)
                adapted_speed = self.ss.adaptSpeed(target_speed, hdr, ctr, min_value=2, max_value=5, he_gain=50.0, ce_gain=30.0, he_thr=0.001, ce_thr=0.002)
                if self.state.v * 3.6 >= adapted_speed:
                    input_brake = (abs(self.state.v * 3.6 - adapted_speed) / 20.0) * 200
                else:
                    input_brake = 0
    ...
    def decision_straight(self, target_idx):
        yaw_list = []
        # for i in range(target_idx+5, target_idx+15):
        #     try:
        #         yaw_list.append(self.path.cyaw[i])
        #     except IndexError:
        #         break

        # mean = np.mean(yaw_list)
        # # print(np.diff(yaw_list))
        # print(np.mean(np.abs(np.diff(yaw_list))))
        # print("----------------------------------")
        # for yaw in yaw_list:
        #     dyaw = mean - yaw
        #     # print(abs(m.degrees(dyaw)))
        #     if abs(m.degrees(dyaw)) > threshold:
        #         return False
        #     else:
        #         pass
        # return True
